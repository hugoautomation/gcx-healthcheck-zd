...
@admin.register(HealthCheckSubscription)
class HealthCheckSubscriptionAdmin(admin.ModelAdmin):
    list_display = ('installation_id', 'customer', 'subscription', 'created_at', 'updated_at')
    list_filter = ('created_at', 'updated_at')
    search_fields = ('installation_id', 'customer__email', 'subscription__id')
...
class HealthCheckReportAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Instance Information', {
            'fields': ('instance_guid', 'installation_id', 'app_guid', 'subdomain', 'admin_email')
        }),
        ('Plan Information', {
            'fields': ('plan', 'stripe_subscription_id', 'version')
        }),
        ('Report Status', {
            'fields': ('is_unlocked', 'stripe_payment_id')
        }),
        ('Report Data', {
            'fields': ('raw_response',)
        }),
...
class HealthCheckMonitoringAdmin(admin.ModelAdmin):
    list_display = ('installation_id', 'subdomain', 'is_active', 'frequency', 'last_check', 'next_check')
    list_filter = ('is_active', 'frequency', 'created_at')
    search_fields = ('installation_id', 'subdomain', 'instance_guid', 'notification_emails')
    readonly_fields = ('created_at', 'updated_at')
    fieldsets = (
        ('Instance Information', {
